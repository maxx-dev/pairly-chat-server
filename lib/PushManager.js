...
class PushManager
{
	...
	async sendWebPush (push,msg)
	{
		...
		webpush.sendNotification(
			...
			.then((res) => {
				con.log('Sent Web-Push to',msg.userId,'for chat', msg.chatId,'code',res.statusCode);
			})
			.catch((err) => {
	...
	async sendPushForChatMsg (msg)
	{
		...
		//con.log('chatMembers',msg.chatId,chatMembers);
		let userIds = chatMembers.reduce( (arr,elem) => arr.concat(elem.user.id),[]);
		con.log('userIds',msg.chatId,userIds);
		...
		con.log(msg.id,msg.chatId,msg.userId,'pushSubs',pushSubs.length);
